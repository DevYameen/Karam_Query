...
import addashimg from "../component/assets/admindash.jpg";
...
const Addash = () => {
  ...
  const total = async () => {
    try {
      const uresponse = await fetch(`${backendUrl}`);
      const ures = await uresponse.json();
      setU(ures.length);

      const qresponse = await fetch(`${backendUrl}/getquery`);
      const qres = await qresponse.json();
      setQ(qres.length);

      const penresponse = await fetch(`${backendUrl}/ad/allpenquery`);
      const penres = await penresponse.json();
      setPen(penres.queries.length || 0);

      const proresponse = await fetch(`${backendUrl}/ad/allproquery`);
      const prores = await proresponse.json();
      setPro(prores.queries.length || 0);

      const comresponse = await fetch(`${backendUrl}/ad/allcomquery`);
